...
### 表情包库
`/sticker` `/stk` `/表情包`
> 管理自动聊天表情包库。上传时会用模型写 emotion/scene。  

- `/sticker upload` `/stku` 附带或回复一张图片后上传
- `/sticker upload force` 忽略查重强制上传
- `/sticker 1` 查看 sid=1
- `/sticker del 1` 删除
- `/all sticker` 列出全部表情包
...
### 语音库
`/voice` `/语音`
> 管理自动聊天预录语音库。tag 填这句语音实际说的台词。  

- `/voice upload 那我今晚真不去` 回复一条语音或附带语音/音频文件后上传
- `/voice upload force 那我今晚真不去` 允许相同 tag 再传一条
- `/voice 1` 试听 vid=1
- `/voice tag 1 新台词` 修改 tag
- `/voice del 1` 删除
- `/voice update` `/更新语音` 扫描待导入语音，并同步角色语音表（缺文件后台慢慢下）
- `/all voice` 列出全部语音
