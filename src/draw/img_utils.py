...
def frames_to_gif(frames: List[Image.Image], duration: int = 100, alpha_threshold: float = 0.5) -> Image.Image:
    from src.utils.utils import TempFilePath
...
def concat_images(images: List[Image.Image], mode) -> Image.Image:
    ...
    if mode == "v":
        ...
        images = [
            img if img.width == max_w else img.resize((max_w, int(img.height * max_w / img.width)))
            for img in images
        ]
    ...
    if mode == "h":
        ...
        images = [
            img if img.height == max_h else img.resize((int(img.width * max_h / img.height), max_h))
            for img in images
        ]
    ...
    if mode == "g":
        max_w = max(img.width for img in images)
        max_h = max(img.height for img in images)
        cols = max(1, int(math.sqrt(len(images))))
        rows = (len(images) + cols - 1) // cols
        ret = Image.new("RGBA", (max_w * cols, max_h * rows))
        for i, img in enumerate(images):
            img = img.convert("RGBA").resize((max_w, max_h))
            x = (i % cols) * max_w
            y = (i // cols) * max_h
...
def limit_image_by_pixels(image: Image.Image | list[Image.Image], max_pixels: int) -> Image.Image | list[Image.Image]:
    ...
    if n is not None:
        ...
        if isinstance(image, Image.Image):
            ...
            return frames_to_gif(frames, int(get_gif_duration(image) * old_n / max(1, len(frames))))
